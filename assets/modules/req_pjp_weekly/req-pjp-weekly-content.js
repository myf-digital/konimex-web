(function () {
    ...
    */
    function initialize() {
        $("#btn-download").click(function () {
            common.removeCookie("module.req.pjp.weekly.update");
            common.direct("req_pjp_weekly/form_download");
        });
    }

    function initializeGrid() {
        let option = {
            title: "Request PJP Weekly",
            toolbar: toolbar(),
            url: common.baseURL("req_pjp_weekly/load"),
            queryParams: {
            ...
            columns: [[
				...
				{field:'salesman_name', title:'Nama Salesman', halign: 'center', align: 'left', sortable:"true", width:200},
    ...
    function toolbar() {
        const btnDownload = commonGrid.btnBuilderText('btn-download', 'primary', 'fa fa-download',' Download PJP Weekly');
        return '<div class="action-grid-toolbar">Week Aktif : ' + paramsession.week_aktif + btnDownload + '</div>';
    }
    ...
    */
    ...
    function updateRow(val) {
        common.loading();
        $.post(common.baseURL("api_v1/call_pjp_detail"), {
            salesmanid: val.salesmanid,
            req_no: val.req_no,
            type: 'weekly'
        }, function (res) {
            val.pjp_detail = res.result || [];
            common.setCookie("module.req.pjp.weekly.update", val);
            common.direct("req_pjp_weekly/form");
        });
    }

    function deleteRow(val) {
        common.dialogDelete(function () {
            $.post("req_pjp_weekly/delete", val, function (data, status) {
                if (200 === data.code) {
