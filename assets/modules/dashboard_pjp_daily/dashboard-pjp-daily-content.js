$(function () {
  ...
  function makeDatasets(row) {
    return [
      {
        label: "Plan Outlet",
        data: [row.plan_outlet],
        backgroundColor: COLORS.plan_outlet.bg,
        borderColor: COLORS.plan_outlet.border,
        borderWidth: 1,
      },
      {
        label: "Unplan Outlet",
        data: [row.unplan_outlet],
        backgroundColor: COLORS.unplan_outlet.bg,
        borderColor: COLORS.unplan_outlet.border,
        borderWidth: 1,
      },
      {
        label: "Plan User",
        data: [row.plan_user],
        backgroundColor: COLORS.plan_user.bg,
        borderColor: COLORS.plan_user.border,
        borderWidth: 1,
      },
      {
        label: "Unplan User",
        data: [row.unplan_user],
        backgroundColor: COLORS.unplan_user.bg,
        borderColor: COLORS.unplan_user.border,
        borderWidth: 1,
      },
    ];
  }
  ...
  function makeCard(canvasId, title, colClass, clickable) {
    ...
    var cardStyle = clickable
      ? "cursor:pointer;transition:box-shadow .2s;"
      : "";
    var cardAttr = clickable
    ...
    return (
      ...
      (clickable
        ...
        : "") +
      "</h5>" +
      '<div class="chart-container"><canvas id="' +
      canvasId +
      '"></canvas></div>' +
      "</div></div>"
    );
          ...
          r.detail_user.forEach(function (d) {
            result.push(
              parentData.concat([
                ...
                d.professional_name || "",
                d.array_product || "",
                fmtDt(d.start_detailing),
                fmtDt(d.end_detailing),
                calcDuration(d.start_detailing, d.end_detailing),
                d.reason || "",
                d.keterangan || "",
              ]),
            );
  ...
  function renderSubareaDetail(data) {
    ...
    var rows = data
      .map(function (r, i) {
        return (
          '<tr style="cursor:pointer;" data-salesmanid="' +
          r.salesmanid +
          '" data-nama="' +
          (r.nama_salesman || "") +
          '">' +
  ...
  function renderSalesmanVisits(result) {
    ...
    function buildVisitTable(visits, label, headerColor) {
      ...
      var rows = visits
        .map(function (r, i) {
          ...
          return (
            ...
            val(r.keterangan) +
            "</td>" +
            '<td><button class="btn btn-xs btn-info btn-visit-detail" data-key="' +
            key +
            '"><i class="fa fa-search"></i> Detail</button></td>' +
            "</tr>"
          );
        })
        .join("");
      return (
        ...
        '<th style="background:' +
        headerColor +
        ';color:#fff;">Customer</th>' +
        '<th style="background:' +
        headerColor +
        ';color:#fff;">Check In</th>' +
        '<th style="background:' +
        headerColor +
        ';color:#fff;">Check Out</th>' +
        '<th style="background:' +
        headerColor +
    ...
    $col.append(
      '<div style="padding-left:10px;padding-right:10px;">' +
        '<div class="dashboard-card">' +
        '<h5 style="margin:0 0 12px;font-weight:600;color:#00a65a;">Plan Outlet <span class="badge" style="background:#00a65a;">' +
  ...
  function showVisitModal(r) {
    ...
    var infoRows = [
      ...
      .map(function (row) {
        return (
          '<tr><td style="width:35%;font-weight:600;background:#f9f9f9;">' +
          row[0] +
          "</td><td>" +
          row[1] +
          "</td></tr>"
