(function () {
  ...
  function previewFiles(data) {
    $("#fileModalLabel").html(`
      Judul: <b>${data.judul}</b><br>
      Brand: <b>${data.nama_brand}</b><br>
      Produk: <b>${data.productid} - ${data.nama_invoice}</b>
    `);
    ...
    if (data && data.files) {
      ...
      files.forEach((f, i) => {
        ...
        if (ext.toLowerCase() == "pdf") {
          ...
          html += `<p>File ${i + 1}</p><div class="text-center"><img src="${f}" width="400"></div>`;
      });
    }
