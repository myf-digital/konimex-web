...
(function () {
  ...
  common.setTitle("Upload Setup Planned");

  let uiForm = $("#fm-upload-setup-planned");
  ...
  function initialize() {
    ...
    uiBtnDownloadTemplate.click(function () {
      ...
      window.location.href = common.baseURL(
        "setup_planned/download_template/" + selectedSalesman,
      );
    });
    ...
    uiBtnUpload.click(function (e) {
      ...
      $.ajax({
        url: common.baseURL("setup_planned/upload"),
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
        success: function (response) {
          ...
          Swal.fire({
            title: "Berhasil!",
            text: res.message || "Data Planned berhasil diupload.",
            icon: "success",
          }).then(() => {
  ...
  function loadSalesman(data) {
    ...
    $.post(
      ...
      function (res) {
        ...
        let options = $.map(res.result, function (o) {
          o.id = o.salesmanid;
          o.text =
