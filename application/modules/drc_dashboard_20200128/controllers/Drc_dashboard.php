...
class Drc_dashboard extends BaseController
{
	...
	function open_preview_act_call_regional_area_city_gff() {
		...
		foreach ($dataheader as $vreg) {
			if ($vreg['header']=='PJP'){
					$html .= '<tr>';
					$html .= '<td style="white-space: nowrap;">'.$vreg['city'].'</a></td>';
					$html .= '<td style="white-space: nowrap;">'.$vreg['header'].'</td>';
					if ($restrict_bu=='GT'){
						$html .= '<td style="white-space: nowrap;text-align:right;">'.number_format($vreg['sls_gt'], 0, '.', ',').' </td>';
					}else{
	...
	function open_detail_sos_regional_area_city() {
		...
		foreach ($datarg as $vreg) {
			if ($vreg['item']=='Toothpaste'){
					$html .= '<tr>';
					$html .= '<td style="white-space: nowrap;"><a href="#" id="hcnational" onclick="open_sos_detail_regional_area_city_outlet('.$vreg['regionalid'].',\''.$vreg['nama_regional'].'\','.$vreg['areaid'].',\''.$vreg['nama_area'].'\','.$vreg['subareaid'].',\''.$vreg['kota'].'\');">'.$vreg['kota'].'</a>&nbsp;<button id="btn-savexls" onclick="save_xls_sos_by_city(\''.$vreg['subareaid'].'\');" type="button" class="btn btn-success fa fa-download btn-xs"></button></td>';
					$html .= '<td style="white-space: nowrap;text-align:center;">'.$vreg['item'].'</td>';
					if ($restrict_bu=='GT'){
						$html .= '<td style="white-space: nowrap;text-align:center;text-align:right;">'.number_format('0', 2, '.', ',').' </td>';
					}else{
	...
	function save_xls_sos_by_city() {
		...
		$qhcreg = $this->db->query(" 
										...
										b.customerid, b.kode_outlet, b.nama_customer,b.alamat, a.salesmanid, c.nama_salesman, c.tipe_sales,
										case when type_sos='P' then 'Toothpaste' else 'Toothbrush' end item, 
										a.qty_sos_gsk qtygsk, a.qty_sos_competitor qtykategori, 
										(a.qty_sos_gsk/a.qty_sos_competitor)*100 sos
										from rekap_sos_detail a left join m_customer b on a.customerid=b.customerid
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('F2', 'Qty GSK')
                    ->setCellValue('G2', 'Qty Kategori')
                    ->setCellValue('H2', '% SOS')
                    ->setCellValue('I2', 'USER PAR-MA')
                    ->setCellValue('J2', 'POSITION')
                    ->setCellValue('K2', 'PERIODE INPUT')
					;
                    $i = 3;
					$no = 1;
					...
					foreach ($datarg as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
                                    ...
                                    ->setCellValue('E'.$i, $vnat['item'])
                                    ->setCellValue('F'.$i, $vnat['qtygsk'])
                                    ->setCellValue('G'.$i, $vnat['qtykategori'])
									->setCellValue('H'.$i, $vnat['sos'])
									->setCellValue('I'.$i, $vnat['salesmanid'].'-'.$vnat['nama_salesman'])
									->setCellValue('J'.$i, $vnat['tipe_sales'])
									->setCellValue('K'.$i, $vnat['periode'])
									;
						$i++;
						$no++;
					}
	...
	function open_detail_product_available_national() {
		...
		*/
		$qhcreg = $this->db->query(" 
		select 'National' area, z.brandid, z.brand, count(1) coverage, sum(z._available) _available, sum(z.fr) fr, sum(z.oos) oos from ( 
				select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area,
						...
						where a.tahun='$tahun' and a.bulan='$bulan' and b.classid='$classid' and a.productid in (select productid from mapping_sku_active where idaccount=b.classid) 
		...
		group by z.brandid, z.brand
		order by z.brandid asc;
								");
		$datarg = $qhcreg->result_array();

		$qchild = $this->db->query(" 
								select 'National' area, z.brandid, z.brand, z.regionalid, z.nama_regional, count(1) coverage, sum(z._available) _available, sum(z.fr) fr, sum(z.oos) oos from ( 
									select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area,
											sum(z.fr) fr, sum(z.oos) oos, case when sum(z.fr)<>sum(z.oos) then 1 else 0 end _available from 
											( 
											select distinct a.customerid,b.kode_outlet,b.nama_customer,b.classid, c.brandid,d.brand, b.regionalid, e.nama_regional, 
									...
									group by z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid,z.brand,z.regionalid, z.nama_regional,z.areaid, z.nama_area 
									order by z.areaid, z.brandid asc) z 
								group by z.brandid, z.brand, z.regionalid, z.nama_regional
	...
	function save_xls_product_availability_national() {
		...
		$qgetaccount = $this->db->query("select nama_class from m_customer_class where classid='$classid';");
        ...
        $objPHPExcel->setActiveSheetIndex(0)
					...
					foreach ($datarg as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
                                    ->setCellValue('A'.$i, $vnat['brand'])
                                    ->setCellValue('B'.$i, $vnat['productid'])
                                    ->setCellValue('C'.$i, $vnat['product_name'])
                                    ->setCellValue('D'.$i, round(($vnat['oos']/$vnat['fr']*100),2).' %')
									;
						$i++;
						$no++;
					}
	...
	function save_xls_product_availability_city() {
		...
		$qhcreg = $this->db->query(" 
									select a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid,
											...
											b.subareaid, g.nama_area city, a.qty_akhir, a.exp_date
									...
									where a.periode between '$periode' and last_day('$periode') and b.classid='$classid' 
										and a.productid in (select productid from mapping_sku_active where idaccount=b.classid)
										and a.productid like '$productid' and c.brandid like '$brandid'
	...
	function save_detail_actual_call_xls() {
		...
		//echo $tempsql;
		$filename = "Data_Actual_Call_".$bulan.$tahun.".xlsx";
		...
		$sqlactcall = $this->db->query(" 
										...
										union all
											...
											where rrk.salesmanid = '$salesmanid' AND
												rrk.periode $tempsql AND rrk_trans.check_in is null          
											group by 
												rrk.periode,
												rrk.customerid,
												cst.nama_customer,
