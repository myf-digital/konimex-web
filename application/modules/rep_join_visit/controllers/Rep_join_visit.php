...
class Rep_join_visit extends BaseController
{
	...
	function open_detail_answer() {
		...
		$html .= '<thead">';
		$html .= '<tr>';
        $html .= '<th style="white-space: nowrap;" colspan=2>Review By '.$data[0]["review_by"].' - TPE '.$data[0]["nama_salesman"].' ('.$data[0]["salesmanid"].')</th>';
		$html .= '</tr>';
		$html .= '</thead">';
		$html .= '<tbody">';
        $i=1;
	...
	function savexls_join_visit_all() {
        ...
        $objPHPExcel->getActiveSheet()->setTitle('Join_Visit_TPE');
