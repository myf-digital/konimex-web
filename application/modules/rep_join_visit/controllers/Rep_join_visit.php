...
class Rep_join_visit extends BaseController
{
	...
	function savexls_join_visit_all() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('I1', 'Review')
                    ->setCellValue('J1', 'Preparation')
                    ->setCellValue('K1', 'Approach')
                    ->setCellValue('L1', 'Regular Shelf Merchandising')
                    ->setCellValue('M1', 'Advance Merchandising on Regular Shelves')
                    ->setCellValue('N1', 'Promo Implementation')
                    ->setCellValue('O1', 'Sell and Secure')
                    ->setCellValue('P1', 'DRC Filling')
                    ;
        ...
        foreach ($datamcs as $value) {
            $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('I'.$row, $value['review'])
                        ->setCellValue('J'.$row, $value['preparation'])
                        ->setCellValue('K'.$row, $value['approach'])
                        ->setCellValue('L'.$row, $value['regular_shelf_merchandising'])
                        ->setCellValue('M'.$row, $value['advance_merchandising_on_regular_shelves'])
                        ->setCellValue('N'.$row, $value['promo_implementation'])
                        ->setCellValue('O'.$row, $value['sell_and_secure'])
                        ->setCellValue('P'.$row, $value['drc_filling'])
						;
			$i++;
            $row++;
        }
