			...
			<div class="jarviswidget jarviswidget-color-blueDark" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false" data-widget-deletebutton="false">
				...
				<div>
					...
					<div class="widget-body">
						...
						<div class="row">
							<div id="id_target" style="margin: 10px;"></div>
						</div>
...
<script type="text/javascript">
       ...
       /* $('#finishdate').datepicker({
        ...
        tbl.datagrid({
            ...
            columns: [[
                    ...
                    {field: 'options', title: 'ACTION', width: 250, halign: 'center', align: 'center', formatter: formatButton},
	...
	function open_target(sid) {
		...
		$.ajax({
			...
			url:"<?php echo site_url('/') . $controller . '/target_detail'; ?>",
			data : "sid="+sid+"&startdate="+startdate,
			success:function(res){
				response = res;
				$('#id_target').html(response);
	...
	function open_productivity(sid) {
		...
		$.ajax({
			...
			url:"<?php echo site_url('/') . $controller . '/open_pvt'; ?>",
			data : "sid="+sid+"&startdate="+startdate,
			success:function(res){
				response = res;
				$('div .modal-header .modal-title').text('Order Detail');			
				$('#modal_data').find('.modal-body').html(response);
				$("#modal_data").modal('show');
	...
	function formatButton(val, row, index) {
		...
		var target_excel = '<?php echo $target_excel; ?>';
		var scpace = '&nbsp;&nbsp;&nbsp;';
		
		return target + product + target_excel; 
	...
	function export_target(sid) {
		var start = document.getElementsByName('startdate')[0].value;
		
		window.location.href = "<?php echo base_url().'index.php/'.$controller.'/export_tagihan/'; ?>"+sid+"/"+start;
		return false;
