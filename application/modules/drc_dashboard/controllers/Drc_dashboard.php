...
class Drc_dashboard extends BaseController
{
	...
	function open_detail_national() {
		...
		$qexecabsensi = $this->db->query("replace into t_sales_absensi (periode, salesmanid, status, checkin, checkout, flag_adjust, keterangan, pjp, `call`, extra_call, crc, promo, competitor, `order`, sos)
	...
	function open_detail_product_available_national() {
		...
		$restrict_bu = $this->input->post("restrict_bu");
		//$periode = $tahun.'-'.$bulan.'-'.$tanggal;
		if($productid=='null'){$sqproductid="";}else{$sqproductid="and a.productid = '$productid'";}
		if($brandid=='null'){$qsbrandid="";}else{$qsbrandid="and c.brandid = '$brandid'";}
	...
	function save_xls_product_availability_city() {
		...
		$qhcreg_old = $this->db->query(" 
									select a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid,
											...
											b.subareaid, g.nama_area city, case when a.date_update is null then 'NA' else a.qty_akhir end qty_akhir, a.exp_date, a.status_aktif, att.status status_att,
		...
		$qhcreg = $this->db->query(" 
									select a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid,
											a.productid,c.nama_invoice product_name,c.brandid,d.brand,
											b.regionalid,e.nama_regional,b.areaid,f.nama_area,
											b.subareaid, g.nama_area city, case when a.status_aktif = 'NA' then 'NA' else a.qty_akhir end qty_akhir, a.exp_date, a.status_aktif,
											att.status as status_att,
											concat(rrkreason.reason,' - ',rrktrans.keterangan) as keterangan
									from t_sales_crc a join m_customer b on a.customerid=b.customerid
										...
										join t_sales_absensi att on att.periode=a.periode and att.salesmanid=a.salesmanid
										join t_sales_rrk_trans rrktrans on rrktrans.periode=a.periode and rrktrans.customerid=a.customerid
										left join t_sales_rrk_reason rrkreason on rrkreason.call_reasonid=rrktrans.call_reasonid 
									where a.periode between '$periode' and last_day('$periode') and a.status_aktif = 'A'
										and a.date_update is not null
										$substrclass 
										and a.productid in (select productid from mapping_sku_active where idaccount=b.classid)
										and a.productid like '$productid' and c.brandid like '$brandid'
        ...
        $objPHPExcel->setActiveSheetIndex(0)
					...
					foreach ($datarg as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
                                    ...
                                    ->setCellValue('L'.$i, $vnat['status_aktif'])
									->setCellValue('M'.$i, $vnat['nama_salesman'].'-'.$vnat['salesmanid'])
                                    ->setCellValue('N'.$i, $vnat['keterangan'])
									;
						$i++;
						$no++;
					}
