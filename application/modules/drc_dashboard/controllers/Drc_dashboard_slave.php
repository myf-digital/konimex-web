...
class Drc_dashboard extends BaseController
{
	...
	function open_detail_national() {
		$dbslave = $this->load->database('slave', TRUE);
		...
		if ($restrict_bu=='GT'){
			$querybu = " tipe_sales='SALESMAN GT' and";
		}else{
			$querybu = " tipe_sales<>'SALESMAN GT' and";
		}
		...
		$qexecabsensi = $dbslave->query("replace into t_sales_absensi (periode, salesmanid, status, checkin, checkout, flag_adjust, keterangan, pjp, `call`, extra_call, crc, promo, competitor, `order`, sos)
										select a.date, a.salesman_id, 
												case when a.type='IST' then 'S' 
												when a.type='ICT' then 'C' 
												when a.type='AHR' then 'H' 
												else 'HF' end tipe, check_in checkin, check_out checkout, b.flag_adjust,
										...
										union
										select a.periode,a.salesmanid,'H' status, min(a.check_in) checkin, max(a.check_out) checkout, b.flag_adjust,'' keterangan, 
										...
										where a.periode between DATE_ADD((select tanggal from m_setup_site), INTERVAL -2 DAY) and DATE_ADD((select tanggal from m_setup_site), INTERVAL -1 DAY)
										group by a.periode,a.salesmanid
										;");

		##query get quota national
       	$qhcnat = $dbslave->query(" 
									...
									left join set_quota_budget d on d.idcity = c.subareaid and d.periode=(select max(periode) from set_quota_budget where periode<='$tahun-12-01')
									$strqueryarea
									union all
									...
									(select count(1) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid $strquery) sfmt,  
										(select count(1) from m_sales_salesman where tipe_sales='SALESMAN GT' and aktif=1 and subareaid=c.subareaid $strquery) sfgt
									from m_area_regional a left join m_area_areasite b on a.regionalid=b.regionalid
									left join m_area_subarea c on c.areaid=b.areaid
		...
		//echo $dbslave->last_query();die();
		
		##query get quota national
		
		$qattnat = $dbslave->query(" 
									...
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid $strquery) sfmt,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN GT' and aktif=1 and subareaid=c.subareaid $strquery) sfgt
									from m_area_regional a left join m_area_areasite b on a.regionalid=b.regionalid
									left join m_area_subarea c on c.areaid=b.areaid
									left join m_sales_salesman d on d.subareaid = c.subareaid
									where d.aktif=1
									...
									case when b.tipe_sales='SALESMAN MT' then 1 else 0 end sfmt,
									case when b.tipe_sales='SALESMAN GT' then 1 else 0 end sfgt
									from 
									...
									");
		//echo $dbslave->last_query();
		$dataatt = $qattnat->result_array();

		##query get performance PAR-MA national
		$qpfgffnat = $dbslave->query(" 
										select '_PJP' header, 
												sum(case when b.tipe_sales='MERCHANDISER' then a.pjp else 0 end) MD,
											sum(case when b.tipe_sales='SPG' then a.pjp else 0 end) SPG,
											sum(case when b.tipe_sales='SALESMAN MT' then a.pjp else 0 end) SLS_MT,
											sum(case when b.tipe_sales='SALESMAN GT' then a.pjp else 0 end) SLS_GT
										...
										select '_CALL' header, 
											...
											sum(case when b.tipe_sales='SALESMAN MT' then a.call else 0 end) _call_SLS_MT,
											sum(case when b.tipe_sales='SALESMAN GT' then a.call else 0 end) _call_SLS_GT
										...
										select '_EXT_CALL' header, 
											...
											sum(case when b.tipe_sales='SALESMAN MT' then a.extra_call else 0 end)  _ext_call_SLS_MT,
											sum(case when b.tipe_sales='SALESMAN GT' then a.extra_call else 0 end)  _ext_call_SLS_GT
										from t_sales_absensi a left join m_sales_salesman b on a.salesmanid=b.salesmanid
										left join m_area_subarea c on b.subareaid=c.subareaid
										left join m_area_areasite d on b.areaid=d.areaid
										left join m_area_regional e on b.regionalid=e.regionalid
	...
	function open_att_detail_regional() {
		...
		$qattnat = $dbslave->query(" 
									select 'PAR-MA Aktif' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
									...
									union all
									select 'PAR-MA Hadir' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
		...
		$qattreg = $dbslave->query(" 
									select x.regionalid, x.nama_regional, 'PAR-MA Aktif' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
									(select a.regionalid,a.nama_regional, b.areaid, b.nama_area, c.subareaid, c.nama_area city,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='MERCHANDISER' and aktif=1 and subareaid=c.subareaid$strquery) md,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SPG' and aktif=1 and subareaid=c.subareaid$strquery) spg,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid$strquery) sfmt,
									...
									union all
									select x.regionalid, x.nama_regional, 'PAR-MA Hadir' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
									(select e.regionalid, e.nama_regional,
									case when b.tipe_sales='MERCHANDISER' then 1 else 0 end md,
									case when b.tipe_sales='SPG' then 1 else 0 end spg,
									case when b.tipe_sales='SALESMAN MT' then 1 else 0 end sfmt,
		...
		foreach ($datanat as $vnat) {
			if ($vnat['item']=='PAR-MA Aktif'){
				...
				}else if ($vnat['item']=='PAR-MA Hadir'){
					$html .= '<tr>';
					$html .= '<td style="white-space: nowrap;"></td>';
					$html .= '<td style="white-space: nowrap;">'.$vnat['item'].'</td>';
					if ($restrict_bu=='GT'){
		...
		foreach ($datarg as $vreg) {
			...
			if ($vreg['item']=='PAR-MA Aktif'){
				...
				}else if ($vreg['item']=='PAR-MA Hadir'){
					$html .= '<tr>';
					$html .= '<td style="white-space: nowrap;"></td>';
					$html .= '<td style="white-space: nowrap;">'.$vreg['item'].'</td>';
					if ($restrict_bu=='GT'){
	...
	function open_preview_act_call_regional() {
		...
		$qpfgffnat = $dbslave->query(" select * from (
		...
		##query get performance PAR-MA regional
		$qpfgffreg = $dbslave->query(" select * from (
	...
	function open_hc_detail_regional_area() {
		...
		$qhchead = $dbslave->query(" 
									...
									(select count(1) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid) sfmt,  
									(select count(1) from m_sales_salesman where tipe_sales='SALESMAN GT' and aktif=1 and subareaid=c.subareaid) sfgt
									from m_area_regional a left join m_area_areasite b on a.regionalid=b.regionalid
									left join m_area_subarea c on c.areaid=b.areaid
									left join m_sales_salesman d on d.subareaid = c.subareaid
									where a.regionalid=$regionalid $strqueryarea
	...
	function open_att_detail_regional_area() {
		...
		$qattheader = $dbslave->query(" 
									...
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid) sfmt,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN GT' and aktif=1 and subareaid=c.subareaid) sfgt
									from m_area_regional a left join m_area_areasite b on a.regionalid=b.regionalid
									left join m_area_subarea c on c.areaid=b.areaid
									left join m_sales_salesman d on d.subareaid = c.subareaid
		...
		$qattreg = $dbslave->query(" 
									select x.regionalid, x.nama_regional, x.areaid, x.nama_area, 'PAR-MA Aktif' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
									(select a.regionalid,a.nama_regional, b.areaid, b.nama_area, c.subareaid, c.nama_area city,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='MERCHANDISER' and aktif=1 and subareaid=c.subareaid) md,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SPG' and aktif=1 and subareaid=c.subareaid) spg,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid) sfmt,
									...
									union all
									select x.regionalid, x.nama_regional, x.areaid, x.nama_area, 'PAR-MA Hadir' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
									(select e.regionalid, e.nama_regional, d.areaid, d.nama_area,
									case when b.tipe_sales='MERCHANDISER' then 1 else 0 end md,
									case when b.tipe_sales='SPG' then 1 else 0 end spg,
									case when b.tipe_sales='SALESMAN MT' then 1 else 0 end sfmt,
	...
	function open_preview_act_call_regional_area() {
		...
		$qheader = $dbslave->query(" select * from (
			select e.regionalid,e.nama_regional,'PJP' header, '1' as _order,
					sum(case when b.tipe_sales='MERCHANDISER' then a.pjp else 0 end) md,
				sum(case when b.tipe_sales='SPG' then a.pjp else 0 end) spg,
				sum(case when b.tipe_sales='SALESMAN MT' then a.pjp else 0 end) sls_mt,
	...
	function open_att_detail_regional_area_city() {
		...
		$qheader = $dbslave->query(" 
									...
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and areaid=c.areaid) sfmt,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN GT' and aktif=1 and areaid=c.areaid) sfgt
									from m_area_regional a left join m_area_areasite b on a.regionalid=b.regionalid
									left join m_area_subarea c on c.areaid=b.areaid
									left join m_sales_salesman d on d.subareaid = c.subareaid
		...
		$qattreg = $dbslave->query(" 
									select x.regionalid, x.nama_regional, x.areaid, x.nama_area, x.subareaid, x.city, 'PAR-MA Aktif' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
									(select a.regionalid,a.nama_regional, b.areaid, b.nama_area, c.subareaid, c.nama_area city,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='MERCHANDISER' and aktif=1 and subareaid=c.subareaid) md,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SPG' and aktif=1 and subareaid=c.subareaid) spg,
									(select if(count(1)>0,count(1)$strpengali,count(1)) from m_sales_salesman where tipe_sales='SALESMAN MT' and aktif=1 and subareaid=c.subareaid) sfmt,
									...
									union all
									select x.regionalid, x.nama_regional, x.areaid, x.nama_area, x.subareaid, x.city, 'PAR-MA Hadir' item, sum(x.md) jmlmd, sum(x.spg) jmlspg, sum(x.sfmt) jmlsfmt, sum(x.sfgt) jmlsfgt from
	...
	function open_preview_act_call_regional_area_city_gff() {
		...
		if ($restrict_bu=='GT'){
			$querybu = " b.tipe_sales='SALESMAN GT' and";
		}else{
			$querybu = " b.tipe_sales not in ('SALESMAN GT','FC','ADMIN') and";
		}
		...
		//dihilangkan -> and a.flag_adjust=0
		$qpfgffreg = $dbslave->query(" select * from (
		...
		##generate div per regional
		$html ='<div class="box-body" id="regional"><h3>Summary Actual Call By PAR-MA City '.$city.'</h3>';
		$html .='<div class="box-footer"><a id="btn-home-form" href="javascript:void(0)" onclick="open_preview_national();" class="btn btn-success fa fa-home"> Home</a>
        ...
        $html .= '<th style="white-space: nowrap;width:300px;" rowspan=2">User PAR-MA</th>';
        $html .= '<th style="white-space: nowrap;text-align:center;" colspan="5">'.date_format($periodedate,"M-Y").'</th>';
		$html .= '</tr>';
	...
	function open_att_detail_regional_area_city_gff() {
		...
		$qattreg = $dbslave->query(" 
									select e.regionalid, e.nama_regional, d.areaid, d.nama_area, c.subareaid, c.nama_area city,b.salesmanid,b.nama_salesman,b.tipe_sales, 
											date_format('$periodedate','%d')-FLOOR(date_format('$periodedate','%d')/7)-(case when date_format('$periodedate','%d') > 25 then (select jml_libur from setup_jumlah_harilibur where tahun='$tahun' and bulan='$bulan') else 0 end) as 'PAR-MA Aktif', 
											count(1) as 'PAR-MA Hadir',
		...
		//echo $dbslave->last_query();
		##generate div per regional
		$html ='<div class="box-body" id="regional"><h3>Summary Man Power Attendance By PAR-MA City '.$city.'</h3>';
		$html .='<div class="box-footer"><a id="btn-home-form" href="javascript:void(0)" onclick="open_preview_national();" class="btn btn-success fa fa-home"> Home</a>
        ...
        $html .= '<th style="white-space: nowrap;text-align:center;" colspan="5">'.date("M-Y",strtotime($periodedate)).'</th>';
		$html .= '</tr>';
		$html .= '<th style="white-space: nowrap; width:150px;text-align:center;">PAR-MA Aktif</th>';
		$html .= '<th style="white-space: nowrap; width:150px;text-align:center;">PAR-MA Hadir</th>';
		...
		foreach ($datarg as $vreg) {
					...
					$html .= '<td style="white-space: nowrap;text-align:center;text-align:right;">'.number_format($vreg['PAR-MA Aktif'], 0, '.', ',').' </td>';
					$html .= '<td style="white-space: nowrap;text-align:center;text-align:right;">'.number_format($vreg['PAR-MA Hadir'], 0, '.', ',').' </td>';
	...
	function open_hc_detail_regional_area_city_gff() {
		...
		if ($restrict_bu=='GT'){
			$querybu = " d.tipe_sales='SALESMAN GT' and";
		}else{
			$querybu = " d.tipe_sales<>'SALESMAN GT' and";
		}
        ...
        $html .= '<th style="white-space: nowrap;width:300px;" ">User PAR-MA</th>';
		$html .= '</tr>';
		$html .= '</thead">';
		$html .= '<tbody">';
        $i=1;
	...
	function open_detail_sos_national() {
		...
		$qexecsosperiod = $dbslave->query("replace into rekap_sos_detail (tahun, bulan, periode, salesmanid, customerid, customerid_m, type_sos, image, 
	...
	function open_detail_product_available_national() {
		...
		if ($restrict_level=='4'){
            ...
            $strqueryarea = " and b.regionalid in (select distinct b.regionalid from  
                                                app_resource a left join app_restrict_location b on a.resource_id=b.resource_id 
                                                where a.username='".$usersession."'
                                                ) ";
		}
		...
		/*$qexecpaperiod = $dbslave->query("replace into rekap_product_available_detail (tahun, bulan, periode, salesmanid, customerid, customerid_m, type_sos, image, 
		...
		*/
		...
		select 'National' area, z.brandid, z.brand, z.categoryid, count(1) coverage, sum(z._available) _available, sum(z.fr) fr, sum(z.oos) oos from ( 
				select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area, z.categoryid,
						...
						( 
						select distinct a.customerid, a.salesmanid, b.kode_outlet,b.nama_customer,b.classid, c.brandid,d.brand, a.productid, c.nama_invoice, b.regionalid, e.nama_regional, 
						...
						where a.tahun='$tahun' and a.bulan='$bulan' $substrclass and a.productid in (select productid from mapping_sku_active where idaccount=b.classid) 
		...
		$qchild = $dbslave->query(" 
								select 'National' area, z.brandid, z.brand, z.categoryid, z.regionalid, z.nama_regional, count(1) coverage, sum(z._available) _available, sum(z.fr) fr, sum(z.oos) oos from ( 
									select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid, z.categoryid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area,
											sum(z.fr) fr, sum(z.oos) oos, case when sum(z.fr)<>sum(z.oos) then 1 else 0 end _available from 
											( 
											select distinct a.customerid, a.salesmanid, b.kode_outlet,b.nama_customer,b.classid, c.brandid, a.productid, c.nama_invoice, c.categoryid, d.brand, b.regionalid, e.nama_regional, 
		...
		$qhcreg = $dbslave->query(" 
									select z.regionalid, z.nama_regional, z.productid,z.product_name, count(1) coverage, sum(z._available) _available,
										sum(z.fr) fr, sum(z.oos) oos from (
											select distinct a.customerid, a.salesmanid, b.kode_outlet,b.nama_customer,b.classid, a.productid,c.nama_invoice product_name,c.brandid,d.brand,
													b.regionalid,e.nama_regional,b.areaid, f.nama_area, b.subareaid, 
													g.nama_area city, a.fr, a.oos, case when a.fr<>a.oos then 1 else 0 end _available  
											...
											where a.tahun='$tahun' and a.bulan='$bulan' $substrclass
	...
	function open_detail_product_available_regional_area() {
		...
		$qhcreg = $dbslave->query(" 
									select z.regionalid, z.nama_regional, z.areaid,z.nama_area, z.productid, z.product_name, count(1) coverage, 
										...
										select distinct a.customerid,a.salesmanid, b.kode_outlet, b.nama_customer,b.classid, a.productid,c.nama_invoice product_name,
	...
	function open_brand_detail_product_available_regional_area() {
		...
		$qchild = $dbslave->query(" 
									select distinct z.brandid, z.brand, z.categoryid, z.regionalid, z.nama_regional, z.areaid, z.nama_area, count(1) coverage, sum(z._available) _available,
											sum(z.fr) fr, sum(z.oos) oos from ( 
									select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid, z.brand, z.categoryid, 
											z.regionalid, z.nama_regional, z.areaid, z.nama_area,
											sum(z.fr) fr, sum(z.oos) oos, case when sum(z.fr)<>sum(z.oos) then 1 else 0 end _available from ( 
										select distinct a.customerid,a.salesmanid,b.kode_outlet,b.nama_customer,b.classid, c.brandid, c.categoryid, d.brand, a.productid, c.nama_invoice,
									...
									group by z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid,z.brand, z.categoryid, z.regionalid, z.nama_regional,z.areaid, z.nama_area 
									order by z.areaid, z.brandid asc) z 
									group by z.brandid,z.brand, z.categoryid, z.regionalid, z.nama_regional,z.areaid, z.nama_area 
	...
	function open_brand_detail_product_available_regional_area_city() {
		...
		$qchild = $dbslave->query(" 
									select distinct z.brandid, z.brand, z.categoryid, z.regionalid, z.nama_regional, z.areaid, z.nama_area, z.subareaid, z.city, count(1) coverage, sum(z._available) _available,
											sum(z.fr) fr, sum(z.oos) oos from ( 
											select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid, z.categoryid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area,
													z.subareaid, z.city, sum(z.fr) fr, sum(z.oos) oos, case when 	sum(z.fr)<>sum(z.oos) then 1 else 0 end _available from ( 
												select distinct a.customerid, a.salesmanid, b.kode_outlet,b.nama_customer, b.classid, c.brandid, d.brand, a.productid, c.nama_invoice, c.categoryid, 
														b.regionalid, e.nama_regional, b.areaid, f.nama_area, b.subareaid, g.nama_area city, a.fr, a.oos
												from t_stock_all_outlet a left join m_customer b on a.customerid=b.customerid left join m_product c on a.productid=c.productid 
													left join ref_brand d on c.brandid=d.brandid left join m_area_regional e on b.regionalid=e.regionalid
													left join m_area_areasite f on b.areaid=f.areaid left join m_area_subarea g on b.subareaid=g.subareaid
											...
											group by z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brandid,z.brand,z.categoryid,z.regionalid, z.nama_regional,z.areaid, z.nama_area 
											order by z.areaid, z.brandid asc) z 
									group by z.brandid,z.brand,z.categoryid,z.regionalid, z.nama_regional,z.areaid, z.nama_area, z.subareaid, z.city
	...
	function save_xls_product_availability_national() {
		...
		if ($classid=='%') {
			...
			$qgetaccount = $dbslave->query("select nama_class from m_customer_class where classid like '$classid';");
		...
		$qhcreg = $dbslave->query(" 
									select z.regionalid, z.nama_regional,z.areaid, z.nama_area, z.subareaid, z.city, z.brand, z.productid, z.nama_invoice product_name, count(1) coverage, sum(z._available) _available, sum(z.fr) fr, sum(z.oos) oos from 
												...
												select distinct z.customerid, z.kode_outlet, z.nama_customer, z.classid, z.brand, z.productid, z.nama_invoice, 
														...
														sum(z.fr) fr, sum(z.oos) oos, case when sum(z.fr)<>sum(z.oos) then 1 else 0 end _available from 
														( 
														select distinct a.customerid,a.salesmanid,b.kode_outlet,b.nama_customer,b.classid, d.brand, c.productid, c.nama_invoice, b.regionalid, e.nama_regional, 
																b.areaid, f.nama_area, b.subareaid,g.nama_area city, a.fr, a.oos
														from t_stock_all_outlet a left join m_customer b on a.customerid=b.customerid left join m_product c on a.productid=c.productid 
															left join ref_brand d on c.brandid=d.brandid left join m_area_regional e on b.regionalid=e.regionalid
															left join m_area_areasite f on b.areaid=f.areaid left join m_area_subarea g on b.subareaid=g.subareaid
	...
	function save_xls_product_availability_city() {
		...
		$qhcreg_old = $dbslave->query(" 
									select a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid,
											...
											GROUP_CONCAT((select reason from t_sales_rrk_reason where call_reasonid=rrktrans.call_reasonid),' - ',rrktrans.keterangan) as keterangan
									from t_sales_crc a join m_customer b on a.customerid=b.customerid
									left join m_product c on a.productid=c.productid
									left join ref_brand d on c.brandid=d.brandid
									left join m_area_regional e on b.regionalid=e.regionalid
									left join m_area_areasite f on b.areaid=f.areaid
									...
									where a.periode between '$periode' and last_day('$periode') $substrclass 
										and a.productid in (select productid from mapping_sku_active where idaccount=b.classid)
										and a.productid like '$productid' and c.brandid like '$brandid'
										...
										and att.status='H'
									group by a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid, a.productid
										;");
		$qhcreg = $dbslave->query(" 
									select a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid,
											a.productid,c.nama_invoice product_name,c.brandid,d.brand,
											b.regionalid,e.nama_regional,b.areaid,f.nama_area,
											b.subareaid, g.nama_area city, case when a.date_update is null then 'NA' else a.qty_akhir end qty_akhir, a.exp_date,
											(select att.status from t_sales_absensi att where att.periode=a.periode and att.salesmanid=a.salesmanid) status_att,
											(select concat(rrkreason.reason,' - ',rrktrans.keterangan) from t_sales_rrk_trans rrktrans 
													left join t_sales_rrk_reason rrkreason on rrkreason.call_reasonid=rrktrans.call_reasonid 
											where rrktrans.periode=a.periode and rrktrans.customerid=a.customerid) as keterangan
									from t_sales_crc a join m_customer b on a.customerid=b.customerid
										left join m_product c on c.productid=a.productid
										left join ref_brand d on d.brandid=c.brandid
										left join m_area_regional e on e.regionalid=b.regionalid
										left join m_area_areasite f on f.areaid=b.areaid
										left join m_area_subarea g on g.subareaid=b.subareaid
										left join m_sales_salesman h on h.salesmanid=a.salesmanid
										left join m_customer_class i on i.classid=b.classid
									where a.periode between '$periode' and last_day('$periode') 
										$substrclass 
										and a.productid in (select productid from mapping_sku_active where idaccount=b.classid)
										and a.productid like '$productid' and c.brandid like '$brandid'
										and b.regionalid='$regionalid' and b.areaid='$areaid' and b.subareaid='$subareaid'
	...
	function save_absensi_detail() {
        ...
        $objPHPExcel = new PHPExcel();
        $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A1', 'Absensi PAR-MA')
                    ->setCellValue('A2', 'No.')
                    ->setCellValue('B2', 'Periode')
                    ->setCellValue('C2', 'Kode PAR-MA')
                    ->setCellValue('D2', 'Nama PAR-MA')
	...
	function save_detail_actual_call_xls() {
		...
		$sqlactcall = $dbslave->query(" 
										select * from (
											...
											case when rrk.customerid is null then 'ExtraCall' 
												...
												when rrk.customerid is not null and (select flag_adjust from t_sales_absensi 
																where salesmanid = '$salesmanid' and periode=rrk_trans.periode)=0 then 'Call' else 'ExtraCall'
												end as flag,
										...
										union all
											...
											where rrk.salesmanid = '$salesmanid' AND
												rrk.periode in (select periode from t_sales_absensi where salesmanid = '$salesmanid' and status in ('H','C') and flag_adjust=0 and periode $tempsql ) AND rrk_trans.check_in is null         
											group by 
												rrk.periode,
												rrk.customerid,
												cst.nama_customer,
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('R2', 'Kode PAR-MA')
                    ->setCellValue('S2', 'Nama PAR-MA')
					;
                    $i = 3;
					$no = 1;
	...
	function save_detail_actual_call_city_xls() {
		...
		$sqlactcall = $dbslave->query(" 
										select * from (
											...
											left join m_area_areasite ar on cst.areaid = ar.areaid
											left join m_area_subarea sa on cst.subareaid=sa.subareaid
											where rrk_trans.salesmanid in (select salesmanid from m_sales_salesman where aktif=1 and subareaid=$subareaid) AND salesamn.tipe_sales not in ('SALESMAN GT','FC','ADMIN') AND
												rrk_trans.periode $tempsql
											group by 
												rrk_trans.periode,
												rrk_trans.customerid,
										...
										union all
											...
											where att.status in ('H','C') and att.flag_adjust=0 and rrk.salesmanid in (select salesmanid from m_sales_salesman where aktif=1 and subareaid=$subareaid) AND salesamn.tipe_sales not in ('SALESMAN GT','FC','ADMIN') AND 
	...
	function save_detail_actual_call_national_xls() {
		...
		if ($restrict_bu=='GT'){
			$querybu = " and tipe_sales='SALESMAN GT'";
		}elseif ($restrict_bu=='MT'){
			$querybu = " and tipe_sales<>'SALESMAN GT'";
		}else{
			$querybu = "";
		...
		$sqlactcall = $dbslave->query(" 
										...
										union all
											...
											left JOIN m_sales_salesman salesamn on rrk.siteid = salesamn.siteid and rrk.salesmanid = salesamn.salesmanid
											left join t_sales_absensi att on att.salesmanid=rrk.salesmanid and att.periode=rrk.periode
											where att.status in ('H','C') and att.flag_adjust=0 and rrk.salesmanid in (select salesmanid from m_sales_salesman where aktif=1 $strqueryarea) AND
												...
												and status in ('H','C') and flag_adjust=0 and periode $tempsql ) AND rrk_trans.check_in is null
											group by
												rrk.periode,
												rrk.customerid,
												cst.nama_customer,
