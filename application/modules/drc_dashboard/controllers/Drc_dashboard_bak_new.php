	...
	function save_detail_actual_call_city_xls() {
		...
		$sqlactcall = $this->db->query(" 
										...
										union all
											...
											where att.status in ('H','C') and rrk.salesmanid in (select salesmanid from m_sales_salesman where aktif=1 and subareaid=$subareaid) AND salesamn.tipe_sales not in ('SALESMAN GT','FC','ADMIN') AND 
