...
class App_dashboard extends BaseController
{
	...
	function get_gmap() {
		...
		foreach ($get_map as $map) {
			...
			$flag = $map['flag'];
			if ($flag == 'EffectiveCall') {
				$icon = base_url().'assets/mapIcon/numbers/png/'.$numtrans.'.png';
				//$icon = base_url().'assets/mapIcon/res/drawable-hdpi/eff_call.png';
			} else if ($flag == 'ExtraCall') {
				$icon = base_url().'assets/mapIcon/res/drawable-hdpi/ext_call.png';
			...
			if ($flag != 'Jadwal') {
				...
				$marker .=' var latlng = new google.maps.LatLng('.$lat.','.$long.');
							...
							\'<table class="table table-striped table-bordered table-condensed" style="white-space: nowrap;">\'+
								...
								$marker .='</td><td valign="center" style="text-align:left;">\'+
									\'<p>Qty Product GSK : '.@$rowsimgsos['qty_sos_gsk'].'</p>\'+
									\'<p>QTY Product Competitor : '.@$rowsimgsos['qty_sos_competitor'].'</p>\'+
							...
							\'</div>\'+	
							\'<hr>\'+
							\'<div><h3>Competitor</h3>\'+
							\'<table class="table table-striped table-bordered table-condensed" style="white-space: nowrap;">\'+
								\'<thead>\'+
									\'<tr style="align:center;">\'+
										\'<th style="text-align:center;">Foto Competitor</th>\'+
										\'<th style="text-align:left;">Keterangan</th>\'+
									\'</tr>\'+
								\'</thead>\'+
								\'<tbody>';
								foreach ($d_img_competitor as $rowsimgcomp){
								$marker .='<tr>\'+
									\'<td valign="center" style="text-align:center;"><img class="img-rounded" onclick="preview_image_competitor(\'+param_link_image+\'); " alt="Image Competitor" style="width:100px; height:100px;" src="'.$urlimage.@$rowsimgcomp['image'].'"></td>\'+
									\'<td valign="center" style="text-align:left;">\'+
									\'<p>Product Competitor : '.@$rowsimgcomp['nama_invoice'].'</p>\'+
									\'<p>Harga Normal : '.@$rowsimgcomp['harga_normal'].'</p>\'+
									\'<p>Harga Promo : '.@$rowsimgcomp['harga_promo'].'</p>\'+
									\'<p>Sewa : '.@$rowsimgcomp['sewa'].'</p>\'+
									\'<p>Tipe Sewa : '.@$rowsimgcomp['tipesewa'].'</p>\'+
									\'<p>Keterangan : '.str_replace(array("\n","\r"),"",str_replace("'","`", @$rowsimgcomp['description'])).'</p>\'+
									\'</td>\'+
									\'</tr>';
								}
							$marker .= '</tbody>\'+
							...
							\'<table class="table table-striped table-bordered table-condensed" style="white-space: nowrap;">\'+
								\'<thead>\'+
									\'<tr style="align:center;">\'+
										\'<th style="text-align:center;">Foto NPD</th>\'+
										\'<th style="text-align:left;">Keterangan</th>\'+
									\'</tr>\'+
								\'</thead>\'+
								\'<tbody>';
								foreach ($d_img_npd as $rowsimgnpd){
									$marker .='<tr>\'+
									\'<td valign="center" style="text-align:center;"><img class="img-rounded" onclick="preview_image_npd(\'+param_link_image+\'); " alt="Image Competitor" style="width:100px; height:100px;" src="'.$urlimage.@$rowsimgnpd['image'].'"></td>\'+
									\'<td valign="center" style="text-align:left;">\'+
									\'<p>Product Competitor : '.@$rowsimgnpd['product_name'].'</p>\'+
									\'<p>Harga Normal : '.@$rowsimgnpd['harga_normal'].'</p>\'+
									\'<p>Keterangan : '.str_replace(array("\n","\r"),"",str_replace("'","`", @$rowsimgnpd['description'])).'</p>\'+
									\'</td>\'+
									\'</tr>';
								}
									$marker .='</tbody>\'+
				...
				if (($lat_sales != "0") and ($long_sales !="0")) {
					...
					$marker_sales .=' var latlng = new google.maps.LatLng('.$lat.','.$long.');
							...
							\'<table class="table table-striped table-bordered table-condensed" style="white-space: nowrap;">\'+
								...
								foreach ($d_img_promo_gsk as $rowsimgpromo){
									if ($rowsimgpromo['tipepromo']=='Gimmick'){
										$marker_sales .=' <tr><td valign="top" style="text-align:center;">';
										$arrimages = explode(',', @$rowsimgpromo['image']);
										foreach ($arrimages as &$images) {
										$marker_sales .='<img class="img-rounded" onclick="preview_image_promo_gsk_gimmick(\'+param_link_image+\'); " alt="Image Promo GSK" style="width:100px; height:100px;" src="'.$urlimage.$images.'"><br><br>';
										}
										$marker_sales .='</td><td valign="center" style="text-align:left;">\'+
										\'<p>Promo : '.@$rowsimgpromo['promo'].'</p>\'+
										\'<p>Tipe : '.@$rowsimgpromo['tipepromo'].'</p>\'+
										\'<p>Stock Awal :'.@$rowsimgpromo['harga_normal'].'</p>\'+
										\'<p>Qty Pasang :'.@$rowsimgpromo['harga_promo'].'</p>\'+
							...
							\'<table class="table table-striped table-bordered table-condensed" style="white-space: nowrap;">\'+
								...
								foreach ($d_img_sos as $rowsimgsos){
									...
									$arrimages = explode(',', @$rowsimgsos['image']);
									foreach ($arrimages as &$images) {
										$marker_sales .= ' <img class="img-rounded" onclick="preview_image_sos(\'+param_link_image+\'); " alt="Image SOS" style="width:100px; height:100px;" src="'.$urlimage.$images.'">';
									}
									$marker_sales .='</td><td valign="center" style="text-align:left;">\'+
										\'<p>Qty Product GSK : '.@$rowsimgsos['qty_sos_gsk'].'</p>\'+
										\'<p>QTY Product Competitor : '.@$rowsimgsos['qty_sos_competitor'].'</p>\'+
			   ...
			   function preview_image_before(siteid,periode,cusid,sales) {
				$.ajax({
				   ...
				   success: function (result) {
					   ...
					   var tempFile = [];
					   $.each(result.images, function (index, value) {
						   var tempFileElemet = {href: "'.URL_IMAGE.'" + value.image, title: "Keteranga : "+value.description};
						   tempFile.push(tempFileElemet);
					   });
   ...
   function preview_image_competitor(siteid,periode,cusid,sales) {
	$.ajax({
	   url: "'.site_url().'/app_dashboard/get_fancy_competitor",
	   type: "POST",
	   async: false,
	   dataType: "json",
	   data: { siteid: siteid, periode:periode, cusid: cusid, sales: sales },
	   success: function (result) {
		   ...
		   var tempFile = [];
		   $.each(result.images, function (index, value) {
			   var tempFileElemet = {href: "'.URL_IMAGE.'" + value.image, title: "Promo Product Competitor : "+value.nama_invoice};
			   tempFile.push(tempFileElemet);
		   });
...
function preview_image_npd(siteid,periode,cusid,sales) {
	$.ajax({
	   url: "'.site_url().'/app_dashboard/get_fancy_npd",
	   type: "POST",
	   async: false,
	   dataType: "json",
	   data: { siteid: siteid, periode:periode, cusid: cusid, sales: sales },
	   success: function (result) {
		   ...
		   var tempFile = [];
		   $.each(result.images, function (index, value) {
			   var tempFileElemet = {href: "'.URL_IMAGE.'" + value.image, title: "NPD : "+value.product_name};
			   tempFile.push(tempFileElemet);
		   });
	...
	function get_gmapaddress() {
		
		$d_posisiadd = $this->dashboard->get_latlong_add();
		//$d_posisiadd = $this->dashboard->get_latlong_add_retailer();
		foreach ($d_posisiadd as $vlist) {
			$idtrx = $vlist['idtrx'];
			//$idtrx = $vlist['idretailer'];
			$latpos = $vlist['latitude'];
			$longpos = $vlist['longitude'];

			$url = 'https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyAxXaXhYT8PzX2BEDwnqy0aeWXOZ5yYzLo&latlng='.trim($latpos).','.trim($longpos).'&sensor=false';
			$json = @file_get_contents($url);
			$data=json_decode($json);
			$status = $data->status;
			if($status=="OK") {
				$sqlupdate = " update temp_longlat_address set alamat='".str_replace("'","`",$data->results[0]->formatted_address)."' 
								,kota='".str_replace("'","`",$data->results[6]->formatted_address)."' where idtrx=$idtrx; ";
				//$sqlupdate = " update temp_longlat_address_retailer set alamat='".str_replace("'","`",$data->results[0]->formatted_address)."' where idretailer=$idtrx; ";
				//$sqlupdate = " update temp_longlat_address_retailer set alamat='".str_replace("'","`",$data->results[0]->formatted_address)."',
								//kota='".str_replace("'","`",$data->results[6]->formatted_address)."' where idretailer=$idtrx; ";
		...
		/*foreach ($d_posisisales as $d_posisiadd) {
			$latpos = $d_posisiadd['latitude'];
			$longpos = $d_posisiadd['longitude'];

			$html = '<script type="text/javascript">
						function initialize(latpos,longpos) {
						var latlng = {lat: latpos, lng: longpos};
