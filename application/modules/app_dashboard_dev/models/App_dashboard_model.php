...
class App_dashboard_model extends CI_Model
{
	...
	function get_record($siteid,$customerid,$salesmanid,$get_date) {
		$q = $this->db->query(" select tcrc.siteid, site.nama_site,
			   ...
			   DATE_FORMAT(tcrc.exp_date,'%d-%m-%Y') exp_date,
			   tcrc.price
				FROM
				t_sales_crc tcrc 
		...
		$return ='\'<table class="table table-striped table-bordered table-condensed" style="white-space: nowrap;">\'+
				\'<thead>\'+
					\'<tr>\'+
						...
						\'<td colspan="6" align="center">'.$get_date.'</td>\'+						
					\'</tr>\'+
					\'<tr>\'+
...
function get_order($siteid,$customerid,$salesmanid,$get_date) {
	...
	function get_node($sid,$get_date) {
		...
		$q = $this->db->query(" select z.* from (
								...
								left join m_customer f on a.customerid=f.customerid and a.salesmanid=f.salesmanid
	...
	function get_image_sos($siteid,$periode,$salesmanid,$customerid) {
		$q = $this->db->query(" select a.qty_sos_gsk, a.qty_sos_competitor, a.sos, group_concat(b.image SEPARATOR ',') image 
								from t_activity_sos a join m_customer_image b on a.transaction_id=b.transaction_id 
	...
	function get_latlong_add() {
		$sql = "  select * from temp_longlat_address where alamat is null limit 0,50; ";
		$data = $this->db->query($sql);
		return $data->result_array();
		//return $data;
	}

	function get_latlong_add_retailer() {
		$sql = "  select * from temp_longlat_address_retailer where alamat is null or kota is null limit 0,50; ";
		$data = $this->db->query($sql);
		return $data->result_array();
		//return $data;
	}
