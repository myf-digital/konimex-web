...
class Rep_gffaktif_model extends CI_Model
{
	...
	function get_salesman($periode,$until,$position,$idjabatan,$usersession,$restrictlevel) {
		...
		$q = $this->db->query("
                                ...
                                where a.salesmanid in (select distinct salesmanid from t_sales_absensi
                                                     where a.tipe_sales like '$val' and periode>=DATE_FORMAT('".$periode."','%Y-%m-%d') and periode<=DATE_FORMAT('".$until."','%Y-%m-%d')) $strquery
