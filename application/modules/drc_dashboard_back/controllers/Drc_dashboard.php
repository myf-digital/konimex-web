...
class Drc_dashboard extends BaseController
{
	...
	function open_att_detail_regional_area_city_gff() {
		...
		$qattreg = $this->db->query(" 
									select e.regionalid, e.nama_regional, d.areaid, d.nama_area, c.subareaid, c.nama_area city,b.salesmanid,b.nama_salesman,b.tipe_sales, 
											date_format('$periodedate','%d')-FLOOR(date_format('$periodedate','%d')/7)-(case when date_format('$periodedate','%d') > 25 then (select jml_libur from setup_jumlah_harilibur where tahun='$tahun' and bulan='$bulan') else 0 end) as 'PAR-MA Aktif', count(1) as 'PAR-MA Hadir', 
		...
		*/
		$qhcreg = $this->db->query(" 
									select 'National' area, z.brandid, z.brand, count(1) coverage, sum(z._available) _available,sum(z.fr) fr, sum(z.oos) oos from (
										select distinct a.customerid,b.kode_outlet,b.nama_customer,b.classid, c.brandid,d.brand, 
		...
		foreach ($datarg as $vreg) {
				...
				$html .= '<td style="white-space: nowrap;text-align:right;text-align:right;">'.number_format(round(($vreg['_available']/$vreg['coverage']*100),2), 0, ',', '.').' %</td>';
				$html .= '</tr>';
				$html .= '<tr>';
				$html .= '<td style="white-space: nowrap;text-align:left;">% OOS</td>';
				$html .= '<td style="white-space: nowrap;text-align:right;text-align:right;">'.number_format(round(($vreg['oos']/$vreg['fr']*100),2), 0, ',', '.').' %</td>';
				$html .= '</tr>';
				$header++;
			}
		$html .= '</tbody>';
	...
	function open_brand_detail_product_available_regional_area() {
		...
		$qchild = $this->db->query(" 
									select z.brandid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area, count(1) coverage, sum(z._available) _available,sum(z.fr) fr, sum(z.oos) oos from (
										select distinct a.customerid,b.kode_outlet,b.nama_customer,b.classid, c.brandid,d.brand, 
													b.regionalid, e.nama_regional, b.areaid, f.nama_area, b.subareaid,g.nama_area city, a.fr, a.oos, 
													case when a.fr<>a.oos then 1 else 0 end _available  
										from t_stock_all_outlet a left join m_customer b on a.customerid=b.customerid
	...
	function open_brand_detail_product_available_regional_area_city() {
		...
		$qchild = $this->db->query(" 
									select z.brandid, z.brand, z.regionalid, z.nama_regional, z.areaid, z.nama_area, z.subareaid, z.city,count(1) coverage, sum(z._available) _available,sum(z.fr) fr, sum(z.oos) oos from (
										select distinct a.customerid,b.kode_outlet,b.nama_customer,b.classid, c.brandid,d.brand, 
													b.regionalid, e.nama_regional, b.areaid, f.nama_area, b.subareaid,g.nama_area city, a.fr, a.oos, 
													case when a.fr<>a.oos then 1 else 0 end _available  
										from t_stock_all_outlet a left join m_customer b on a.customerid=b.customerid
	...
	function save_xls_product_availability_city() {
		...
		$qhcreg = $this->db->query(" 
									select a.periode, a.salesmanid, h.nama_salesman, a.customerid,b.kode_outlet,b.nama_customer,b.classid,i.nama_class,b.typeid,
											...
											b.subareaid, g.nama_area city, a.qty_akhir  
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('G2', 'Kode Product')
                    ->setCellValue('H2', 'Product')
                    ->setCellValue('I2', 'Stock')
                    ->setCellValue('J2', 'PAR-MA')
					;
                    $i = 3;
					$no = 1;
					...
					foreach ($datarg as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
                                    ...
                                    ->setCellValue('G'.$i, $vnat['productid'])
                                    ->setCellValue('H'.$i, $vnat['product_name'])
                                    ->setCellValue('I'.$i, $vnat['qty_akhir'])
									->setCellValue('J'.$i, $vnat['nama_salesman'].'-'.$vnat['salesmanid'])
									;
						$i++;
						$no++;
					}
	...
	function save_detail_slob_regional_area_city_xls() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('M2', 'Qty Ed')
                    ->setCellValue('N2', 'Value')
					;
                    $i = 3;
					$no = 1;
					...
					foreach ($datanat as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
									...
									->setCellValue('L'.$i, $vnat['product_name'])
									->setCellValue('M'.$i, $vnat['qty_ed'])
									->setCellValue('N'.$i, ($vnat['hjp']*$vnat['qty_ed']));
						$i++;
						$no++;
					}
        // Redirect output to a client's web browser (Excel2007)
