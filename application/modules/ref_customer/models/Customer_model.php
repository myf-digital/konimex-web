...
class Customer_model extends CI_Model
{
    ...
    public function get_template_data($regionalid = null, $areaid = null, $subareaid = null, $usersession = null, $restrict_level = null)
    {
        ...
        $this->db->select('r.regionalid, r.nama_regional, a.areaid, a.nama_area');
        $this->db->from('m_area_regional r');
        $this->db->join('m_area_areasite a', 'r.regionalid = a.regionalid', 'left');

        if (!empty($regionalid)) {
            $reg_arr = explode(',', $regionalid);
            $this->db->where_in('r.regionalid', $reg_arr);
        }
        if (!empty($areaid)) {
        ...
        $this->db->order_by('r.nama_regional, a.nama_area', 'ASC');
    ...
    public function import_excel($fileTmp, $usersession, $restrict_level = null)
    {
        ...
        try {
            ...
            foreach ($sheet as $index => $row) {
                ...
                $alamat = isset($row[8]) ? trim($row[8]) : '';
                $nama_professional = isset($row[9]) ? trim($row[9]) : '';
                ...
                if (!empty($nama_professional)) {
                    ...
                    foreach ($prof_names as $prof_name) {
                        ...
                        $spesialisasi_name_input = isset($row[10]) ? trim($row[10]) : '';
                        ...
                        $type_input = isset($row[11]) ? trim($row[11]) : null;
