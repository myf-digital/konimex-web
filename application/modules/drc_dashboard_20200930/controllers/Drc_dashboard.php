...
class Drc_dashboard extends BaseController
{
	...
	function save_xls_product_availability_national_row_data() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('K2', 'PAR-MA')
                    ->setCellValue('L2', 'Regional')
                    ->setCellValue('M2', 'Area')
                    ->setCellValue('N2', 'City')
					;
                    $i = 3;
					$no = 1;
					...
					foreach ($datarg as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
									...
									->setCellValue('K'.$i, $vnat['nama_salesman'].'-'.$vnat['salesmanid'])
                                    ->setCellValue('L'.$i, $vnat['nama_regional'])
                                    ->setCellValue('M'.$i, $vnat['nama_area'])
                                    ->setCellValue('N'.$i, $vnat['city'])
									;
						$i++;
						$no++;
					}
	...
	function save_detail_actual_call_xls() {
		...
		$sqlactcall = $this->db->query(" 
										select * from (
											...
											r.nama_regional, ar.nama_area, sa.nama_area city,
        ...
        $objPHPExcel->setActiveSheetIndex(0)
					...
					foreach ($resdata as $vnat) {
                        $objPHPExcel->setActiveSheetIndex(0)
									...
									->setCellValue('N'.$i, $vnat['jarak'])
									->setCellValue('O'.$i, $vnat['flag'])
									;
						$i++;
						$no++;
					}
